    ...
    <div className="min-h-screen bg-green-950 pb-20 overflow-x-hidden relative">
            ...
            <div className="hidden sm:flex items-center gap-2 bg-black/20 px-4 py-1.5 rounded-full border border-white/10">
              <span className="text-white text-[10px] font-black uppercase tracking-tighter">
                {selectedTopic ? (
                    ...
                    <>Materia: <span className="text-amber-400">{selectedSubject.toUpperCase()}</span></>
                )}
              </span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-white font-bold text-xs hidden xs:block">{user.name}</span>
              <div className="w-9 h-9 rounded-full border-2 border-amber-400 bg-white flex items-center justify-center text-xl">{user.avatar}</div>
            </div>
          </div>
        </nav>
      )}
      ...
      {view === AppView.DASHBOARD && (
        <div className="fixed bottom-6 right-6 z-50">
          <button onClick={handleLogout} className="flex items-center justify-center bg-rose-600 hover:bg-rose-500 text-white w-14 h-14 rounded-full shadow-xl shadow-rose-900/40 active:scale-90 transition-all border-4 border-rose-800">
